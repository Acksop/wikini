...
chaque page est reprÃ©sentÃ©e par une ellipse
...
sa couleur est fonction du nombre de modifications enregistrÃ©es (tend vers le rouge)
*/
...
{
    //mesure le nombre d'intervenants diffÃ©rents sur une page
...
if ($pagesM = $this->LoadAll($query))
{
    foreach ($pagesM as $page)
    {
        ...
        $col = $p[$id]['nbm'] * 4; // couleur proportionnelle, a ajuster en fonction de l'activitÃ© globale
        if($col > 255) { $col = 255 ;}
...
if ($pagesF = $this->LoadAll($query))
{
    foreach ($pagesF as $page)
    {
        ...
        if(!is_array($p[$id])) // variable page non encore initialisÃ©e = page non enregistrÃ©e
        {
            $p[$id] = array();
            $p[$id]['n'] = $page['to_tag'];
            $p[$id]['nbin'] = 0;
...
if ($pagesT = $this->LoadAll($query))
{
    foreach ($pagesT as $page)
    {
        ...
        if(!is_array($p[$id]))
        {
            ...
            $p[$id]['nbout'] = 0;
            $p[$id]['nbm'] = 0;
            $p[$id]['col'] = "rgb(0,255,0)"; // page pas encore enregistrÃ©e sans lien entrant
        }
...
<?php
/*
Description: Handler pour l'affichage SVG du rÃ©seau de liens d'une page wikini
auteurs: Yann Le Guennec - Charles Nepote
version : 0.0.5

ParamÃ¨tres utilisateurs:
script = on : active le script de dÃ©placement des mots
subnet = off : desactive le rÃ©seau de liens secondaires
...
$n = 0; // nombre de pages liÃ©es Ã  la page courante
$p = array();
...
if ($pagesFrom = $this->LoadPagesLinkingTo($this->getPageTag()))
{
    ...
    {
        $id = $pageFrom["tag"];
        if($id != $this->getPageTag())// exclusion des liens de la page vers elle mÃªme
        {
            if(!is_array($p[$id])) // si le neud n'est pas deja enregistrÃ©
            {
                $p[$id]['n'] = $id;
                $p[$id]['c'] = 'in';
                $p[$id]['w'] = 10;
...
// taille de police
// la taille de police augmente de 1 em Ã  chaque 30 noeuds supplÃ©mentaires dans le graphe
$fontsize = ceil($n/30);
...
// attribution de coordonnÃ©es alÃ©atoires
...
// coordonnÃ©es du centre du rectangle de la page courante
...
$svg_head = "";     // dÃ©but du fichier svg
$svg_links = "";    // couche de carrÃ©s cliquables
...
$svg_head =  "<?xml version=\"1.0\"  encoding=\"iso-8859-1\" ?> \r\n";
$svg_head .= "<!-- 
    Cartographie des ".$n." pages liÃ©es Ã  la page ".$this->getPageTag().". 
    GÃ©nÃ©rÃ© par svg.php - version 0.0.4. 
    Le ".date("Y-m-d H:i:s")." 
$affich_param 
--> \n";
...
//variables pour les identifiants des Ã©lÃ©ments
$i = 0;
$j = 0;
...
foreach ($p as $page)
{
            ...
            if ($retroLiens = $this->LoadPagesLinkingTo($nompage)) //backlinks
            {
                foreach ($retroLiens as $retroLien)
                {
                    ...
                    if(is_array($p[$id]) && $p[$id]['n'] != $this->getPageTag())
                    {
                        ...
                        $page['w']++; //incrementation de la taille de la page liÃ©e
                        $i++;
                    }
                }
            }
    ...
    //ajuste les coords de ligne au centre des carrÃ©s
